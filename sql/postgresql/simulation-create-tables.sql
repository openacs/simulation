...
create table sim_objects (
    sim_object_id	integer		constraint sim_objects_fk
					  references acs_objects
					constraint sim_objects_pk
					  primary key,
    parent_sim_object   integer		constraint sim_objects_parent_fk
					  references sim_objects,
    description		text
);
...
select acs_object_type__create_type (
	...
	'sim_objects', 				-- table_name
	'sim_object_id',      			-- id_column
	null,				        -- package_name
	'f',				        -- abstract_p
	null,				        -- type_extension_table
	'sim_object__name' 			-- name_method
	);

create table sim_workflow_object_map (
    workflow_id		integer		constraint sim_workflows_object_map_fk
					  references workflows,
    sim_object_id	integer		constraint sim_workflows_object_map_2_fk
 					  references sim_objects,
    constraint sim_workflow_object_map_pk
      primary key (workflow_id, sim_object_id)
);
